...
test('keeps the settings preview close to the reader dialogue layer', () => {
  ...
  assert.match(settings, /\.preview-screen \{[^}]*width: 288px;[^}]*height: 108px;/)
  assert.match(settings, /\.preview-scroll \{[^}]*left: 0px;[^}]*width: 288px;/)
  assert.match(settings, /这是一段较长的对白预览/)
})

test('uses only the Xiaomi Band 10 Pro 336 by 480 layout', () => {
  for (const source of [index, detail, settings, saves, about]) {
    assert.doesNotMatch(source, /@media \(shape: rect\)|303px/)
    assert.match(source, /\.page \{ width: 336px; height: 480px;/)
  }
  assert.match(index, /\.home-background \{[^}]*width: 336px;[^}]*height: 480px;/)
  assert.match(detail, /\.dialogue-panel \{[^}]*top: 306px;[^}]*height: 174px;/)
})
...
test('reloads saved text settings after returning to an existing reader page', () => {
  ...
  assert.match(detail, /onShow\(\) \{[^}]*this\.loadSettings\(\)/)
