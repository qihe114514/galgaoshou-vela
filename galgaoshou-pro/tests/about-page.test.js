...
test('uses a transparent static text image inside the scroll area', async () => {
  ...
  assert.match(about, /\.group-section \{[^}]*width: 288px;[^}]*flex-direction: column;[^}]*align-items: center;/)
  assert.match(about, /\.group-qr \{[^}]*width: 112px;[^}]*height: 112px;/)
  assert.doesNotMatch(about, /content-inner/)
  assert.match(about, /\.content \{[^}]*width: 288px;[^}]*height: 350px;[^}]*flex-direction: column;[^}]*flex-wrap: nowrap;/)
  assert.match(about, /\.about-text \{[^}]*width: 288px;[^}]*height: 800px;/)
