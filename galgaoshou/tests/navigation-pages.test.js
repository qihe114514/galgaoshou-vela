...
test('keeps save routing and new pages local to the device', () => {
  ...
  assert.match(saves, /<list id="saves-list" class="save-list \{\{motionStaticClass\}\}"/)
  ...
  for (const source of [settings, saves, about]) assert.match(source, /page-motion/)
  assert.match(index, /from '@system\.folme'/)
...
test('keeps the settings preview close to the reader dialogue layer', () => {
  assert.match(settings, /<div id="settings-preview" class="preview-screen \{\{motionStaticClass\}\}">[\s\S]*<div class="preview-panel"><\/div>[\s\S]*class="preview-name"[\s\S]*<scroll class="preview-scroll"/)
  ...
  assert.match(settings, /\.preview-screen \{[^}]*width: 192px;/)
  assert.match(settings, /\.preview-scroll \{[^}]*left: 0px;[^}]*width: 192px;/)
