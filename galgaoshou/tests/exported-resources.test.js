...
test('exports compact wide PNG scenes and transparent PNG layers', async () => {
  ...
  for (const asset of assets) {
    ...
    if (asset.kind === 'background') {
      ...
      assert.equal(metadata.width, 138)
      assert.equal(metadata.height, 358)
      ...
      assert.equal(metadata.height, 490)
      assert.equal(pngColorType(file), 3, `${asset.key} must be a palette PNG`)
...
test('exports the static home screen, circular icon, and transparent logo', async () => {
  assert.deepEqual((await sharp('src/common/home.png').metadata()).width, 192)
  assert.equal((await sharp('src/common/home.png').metadata()).height, 490)
  const icon = await sharp('src/common/icon-128.png').ensureAlpha().raw().toBuffer({ resolveWithObject: true })
  assert.deepEqual({ width: icon.info.width, height: icon.info.height }, { width: 128, height: 128 })
  assert.equal((await sharp('src/common/icon-128.png').metadata()).isPalette, false)
  ...
  assert.match(optimizeHome, /icon-128\.png/)
  assert.match(optimizeHome, /icon\.png'\)\)\.resize\(128, 128/)
  const logo = await sharp('src/common/logo.png').metadata()
  assert.deepEqual({ width: logo.width, height: logo.height }, { width: 163, height: 93 })
