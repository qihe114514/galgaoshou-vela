...
test('ships only the trial navigation flow', () => {
  assert.equal(manifest.versionCode, 61)
...
test('opens the purchase page from static home actions', () => {
  ...
  assert.match(home, /\.exit-action[^}]*width: 220px[^}]*background-color: #ead1da[^}]*color: #722c4b/)
})
...
test('uses only the fixed Xiaomi Band 10 Pro 336 by 480 layout', () => {
  assert.equal(manifest.config.designWidth, 336)
  for (const page of [home, detail, end, settings, about]) {
    assert.match(page, /\.page \{ width: 336px; height: 480px;/)
    assert.doesNotMatch(page, /@media \(shape: rect\)/)
  }
  assert.match(home, /src="\/common\/home-pro\.png"/)
  assert.ok(fs.existsSync('src/common/home-pro.png'))
  assert.match(detail, /\.dialogue-panel \{[^}]*top: 306px;[^}]*width: 336px;[^}]*height: 174px;/)
